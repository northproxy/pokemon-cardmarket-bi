...
`requests.get`/`requests.post` and FTP_TLS are mocked. These tests verify
the orchestration logic (call order, correct filenames, correct paths, and
notification behavior) rather than actual connectivity.
"""
import sys
import unittest
...
class TestBuildMessages(unittest.TestCase):
    def test_success_message_includes_date_filename_size(self):
        msg = dpg.build_success_message(
            date(2026, 7, 3), "price_guide_6_2026-07-03.json", 123456
        )
        self.assertIn("2026-07-03", msg)
        self.assertIn("price_guide_6_2026-07-03.json", msg)
        self.assertIn("123,456", msg)  # thousands separator
        self.assertIn("TBD", msg)  # last-item field is a known placeholder

    def test_failure_message_includes_exception_text(self):
        msg = dpg.build_failure_message(ValueError("boom"))
        self.assertIn("boom", msg)


class TestNotifyTelegram(unittest.TestCase):
    @patch("src.ingestion.download_price_guide.requests.post")
    def test_skips_silently_when_not_configured(self, mock_post):
        with patch.object(dpg.config, "TELEGRAM_BOT_TOKEN", None), \
             patch.object(dpg.config, "TELEGRAM_CHAT_ID", None):
            dpg.notify_telegram("hello")
        mock_post.assert_not_called()

    @patch("src.ingestion.download_price_guide.requests.post")
    def test_posts_when_configured(self, mock_post):
        with patch.object(dpg.config, "TELEGRAM_BOT_TOKEN", "fake-token"), \
             patch.object(dpg.config, "TELEGRAM_CHAT_ID", "fake-chat-id"):
            dpg.notify_telegram("hello")
        mock_post.assert_called_once()
        url, kwargs = mock_post.call_args.args[0], mock_post.call_args.kwargs
        self.assertIn("fake-token", url)
        self.assertEqual(kwargs["data"]["chat_id"], "fake-chat-id")
        self.assertEqual(kwargs["data"]["text"], "hello")

    @patch("src.ingestion.download_price_guide.requests.post", side_effect=Exception("network down"))
    def test_never_raises_even_if_request_fails(self, mock_post):
        # A Telegram failure must never propagate and fail the pipeline.
        with patch.object(dpg.config, "TELEGRAM_BOT_TOKEN", "fake-token"), \
             patch.object(dpg.config, "TELEGRAM_CHAT_ID", "fake-chat-id"):
            dpg.notify_telegram("hello")  # should not raise


class TestRunOrchestration(unittest.TestCase):
    @patch("src.ingestion.download_price_guide.notify_telegram")
    ...
    def test_uploads_base_filename_when_nothing_exists_on_ftp(
        ...
        # Success notification fires exactly once, on the happy path.
        mock_notify.assert_called_once()
        self.assertIn("2026-07-03", mock_notify.call_args.args[0])

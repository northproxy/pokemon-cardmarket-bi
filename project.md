...
| 0.7 | 2026-07-19 | Corrected source_created_at throughout: a real price_guide_6.json sample confirmed Cardmarket provides a root-level createdAt timestamp, contradicting the v0.4 assumption in 03-data-dictionary.md that no source-provided timestamp exists. source_created_at now stores that value (parsed once per file, applied to every row) instead of the pipeline's download time. 02-data-model.md (v0.6), 03-data-dictionary.md (v0.6), and 04-etl-pipeline-design.md (v0.7) updated to match; logged as DECISIONS.md §13 in the code repository. src/transform/price_guide.py already implements the corrected behavior. |
